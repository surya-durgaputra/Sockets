...
namespace SocketsServerStarter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // we need to put this in try catch block
            // because when the client side process closes, it will cause an exception 
            // in this listener bound socket. We need to catch that exception so 
            // we can exit gracefully, without a crash
            try
            {
                ...
                // put the follwing in while loop so that we keep receiving
                while (true)
                {
                    ...
                    // now lets send some data back to the client
                    client.Send(buff);

                    // since infinite loops are not good, we put a breaking condition here
                    if (receivedText == "x")
                    {
                        break;
                    }

                    // we need to clean the array buff before we can use it again otherwise our data will be garbled
